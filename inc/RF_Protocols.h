...
#define Quigg
SI_SEGMENT_VARIABLE(PROTOCOL_BUCKETS(Quigg)[], static uint16_t, SI_SEG_CODE) = { 700, 1400, 81000 };
SI_SEGMENT_VARIABLE(PROTOCOL_START(Quigg)[], static uint8_t, SI_SEG_CODE) = {  HIGH(0) };
...
SI_SEGMENT_VARIABLE(PROTOCOL_END(Quigg)[], static uint8_t, SI_SEG_CODE) = {  LOW(2) };
#endif
...
SI_SEGMENT_VARIABLE(PROTOCOL_DATA[], static struct BUCKET_PROTOCOL_DATA, SI_SEG_CODE) =
{
		 ...
		 */
		{
			...
			{ &PROTOCOL_END(Quigg), ARRAY_LENGTH(PROTOCOL_END(Quigg)) },
			20
		},
#endif
};
